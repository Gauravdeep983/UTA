...
public class Problem3Class {
	...
	public void operateBinoculars(state currentState, int D, int G, int P, int Z) {
		switch (currentState) {
			...
			case U:	{if (Z==1)
						{B=1;I=0;T=0;X=3;nextState=state.X5;}
					else
						if (P==1)
							{B=0;I=0;T=0;X=0;nextState=state.L;}
						else
							{B=1;I=0;T=0;X=2;nextState=state.U;}}
			case X5:	{if (G==1 || B==1)
							{B=1;I=0;T=0;X=2;nextState=state.U;}
						else 
							if (Z==1)
								...
								{B=1;T=1;T=0;X=2;nextState=state.N;}							}
			case X10:	{if (P==1)
							{B=1;T=0;T=0;X=1;nextState=state.OFF;}
						else
							{B=1;T=0;T=0;X=3;nextState=state.X10;}}
			case N:     {if (D==1)
			...
			case L:     {if (P==1)
							...
							{B=1;I=0;I=1;X=0;nextState=state.L;}}
			}
	}
